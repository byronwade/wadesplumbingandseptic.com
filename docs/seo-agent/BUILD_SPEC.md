...
The sidecar SHALL retain one compiled Eve schedule named `audit` at `17 16 * * 1` UTC. In observe mode it dispatches audit only. In propose mode the same schedule selects the deterministic `proposal-YYYY-MM-DD` job, researches a bounded post, and opens one draft PR through Vercel Connect GitHub. It SHALL fail before a durable Eve session starts unless propose mode, enabled mutation mode, a disabled kill switch, and `SEO_AGENT_ENABLE_GITHUB=true` all match. The draft path cannot be invoked from a public route and does not add merge, deployment, main-write, or force-push authority.

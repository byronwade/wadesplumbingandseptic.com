...
For an initial live draft proof, the sidecar SHALL retain one compiled Eve schedule named `audit` at `17 16 * * 1` UTC. It dispatches an audit by default. A Vercel administrator may invoke that single registered schedule with the Vercel CLI, which supplies the platform application principal without disclosing `CRON_SECRET`. The server-only `SEO_AGENT_NATIVE_SCHEDULE_JOB=proposal` override may select the deterministic `proposal-YYYY-MM-DD` job for one exact proof only. It SHALL fail before a durable Eve session starts unless propose mode, enabled mutation mode, a disabled kill switch, the human approval flag, the integration-test flag, the exact approved run ID, and a reviewed `audit-YYYY-MM-DD` precondition run ID all match. The override must be restored to `audit` immediately after the run. It is not a standing publication schedule, cannot be invoked from a public route, and does not add merge, deployment, main-write, or force-push authority.

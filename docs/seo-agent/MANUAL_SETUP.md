...
**Cost control.** DataForSEO is the only metered, pay-per-call adapter Eve has; every other adapter is free or quota-based. It gets its own hard ceiling, separate from and in addition to Eve's general 30-external-request-per-run budget: `maxDataForSeoRequestsPerRun` in `automation/seo-agent/src/constants.mjs`, currently `5`. Any call past that limit throws immediately rather than reaching the network, and it is versioned code (a reviewed PR), not an environment variable, so it cannot be silently loosened. Search volume lookups are also capped at 20 keywords per call. Today none of this can run unattended at all: the live-probe Cron path is a manual-trigger-only placeholder schedule (`0 0 1 1 *`) that requires an owner to arm `SEO_AGENT_LIVE_READS_APPROVED` for one exact run ID, trigger it by hand, then disarm it — see the live proof steps below. Before wiring `searchVolume`/`backlinksSummary` into the automatic weekly audit, keep the same discipline: a small, deliberately curated keyword/domain list per run, not a bulk sweep.

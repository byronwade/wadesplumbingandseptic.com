...
| Image sourcing   | [ ] Prefer first-party work photos; review draft PR Images section for online staged assets under `public/images/sourced/` and reject photoreal AI filler; optional Unsplash/Pexels/Pixabay/Flickr/Europeana keys; AI line-art flag only after review; treat Wikidata/Nominatim/Grokipedia lines as soft context only (Grokipedia is public HTML only, not `/api/`) | Expanded open APIs + Grokipedia soft research offline `MOCK_VERIFIED`; live keyed proofs pending credentials |

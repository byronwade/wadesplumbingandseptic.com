...
1. [ ] Request/confirm Vercel Services availability, then link one Vercel project at the repository root and select the Services framework. Keep the committed root `vercel.json` (rewrite-routed `site` + `eve_seo_agent`); do not create a second sidecar project.

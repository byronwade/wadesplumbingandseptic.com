...
Activation state: root `vercel.json` declares the reviewed Services topology on the feature branch. The linked project must remain on the Next.js Framework Preset until this branch is ready for its first preview. Then switch its Framework Preset to Services immediately before deploying the reviewed branch. This is one project and one deployment, not a second Vercel project.

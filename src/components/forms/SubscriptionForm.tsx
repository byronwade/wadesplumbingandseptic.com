		...
		<form onSubmit={handleFormSubmit}>
			<div className="items-center mx-auto mb-3 space-y-4 max-w-screen-sm sm:flex sm:space-y-0">
				<div className="relative w-full">
					...
					<input placeholder="Type in your email here..." required type="text" name="email" id="email" autoComplete="email" className="block p-3 pl-10 w-full text-base text-gray-900 bg-white rounded border border-gray-300 sm:rounded-none sm:rounded-l focus:ring-brand-500 focus:border-brand-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-brand-500 dark:focus:border-brand-500" />
				</div>
				<div>
					<button disabled={succeeded} type="submit" className="py-3 px-5 w-full text-sm font-medium text-center text-white rounded border cursor-pointer bg-brand border-brand-600 sm:rounded-none sm:rounded-r hover:bg-brand-600 focus:ring-4 focus:ring-brand-300 dark:bg-brand-600 dark:hover:bg-brand-700 dark:focus:ring-brand-800">
						Subscribe
					</button>
				</div>
			</div>

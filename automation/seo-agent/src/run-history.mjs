...
export function collectRecordedRunIds({ repoRoot }) {
	...
	for (const entry of readdirSync(runsRoot, { withFileTypes: true })) {
		...
		if (!existsSync(manifestPath))
			throw new Error(`Run history is missing its manifest: ${entry.name}`);
		let manifest;
		try {
		...
		if (manifest.run_id !== entry.name)
			throw new Error(
				`Run history manifest ID does not match its directory: ${entry.name}`,
			);
		if (runIds.has(manifest.run_id))
			throw new Error(
				`Run history contains a duplicate run ID: ${manifest.run_id}`,
			);

...
export function scoreOpportunity(input) {
	...
	const cannibalizationRisk = score(
		input.cannibalization_risk,
		"cannibalization_risk",
	);
	const safetyRisk = score(input.safety_risk, "safety_risk");
	return (
		demand * intentFit * businessEvidence * feasibility -
		cannibalizationRisk -
		safetyRisk
	);
}

export function assignQueryOwner({
	ownership,
	queryCluster,
	ownerUrl,
	intent,
	...
	if (!ownerUrl?.startsWith("/"))
		throw new Error("Query owner must be an internal canonical path");
	if (!Array.isArray(evidenceIds) || evidenceIds.length === 0)
		throw new Error("Query ownership requires evidence");
	if (ownership.some((item) => item.query_cluster === queryCluster))
		throw new Error(`Query cluster already has an owner: ${queryCluster}`);
	return [
		...ownership,
		{
...
export function validateProposal(proposal, { inventory, approvedFacts }) {
	if (
		!proposal.id ||
		!proposal.query_cluster ||
		!proposal.owner_url ||
		!proposal.rollback_target
	)
		throw new Error("Proposal is missing required fields");
	if (!proposal.owner_url.startsWith("/"))
		throw new Error("Proposal owner URL must be internal");
	if (!proposal.existing_page_assessment)
		throw new Error("Proposal must evaluate an existing page first");
	if (!proposal.duplicate_cannibalization_assessment)
		throw new Error(
			"Proposal requires duplicate and cannibalization assessment",
		);
	if (
		!Array.isArray(proposal.evidence_ids) ||
		proposal.evidence_ids.length === 0
	)
		throw new Error("Proposal requires evidence");
	if (
		proposal.new_url &&
		proposal.existing_page_assessment !== "EXISTING_INSUFFICIENT"
	)
		throw new Error(
			"New URLs require an explicit existing-page insufficiency decision",
		);
	if (
		proposal.new_url &&
		(!proposal.new_url.startsWith("/") ||
			inventory.pages.some((page) => page.url === proposal.new_url))
	)
		throw new Error("New URL duplicates an existing canonical page");
	if (
		!inventory.pages.some((page) => page.url === proposal.owner_url) &&
		!proposal.new_url
	)
		throw new Error("Proposal owner is not in the page inventory");
...
export function validateMarkdownDraft(markdown) {
	...
	for (const field of [
		...
		if (!match[1].includes(field))
			throw new Error(`Markdown draft missing ${field}`);
	}
	if (!classifyUntrustedText(markdown).accepted)
		throw new Error("Markdown draft contains disallowed content");

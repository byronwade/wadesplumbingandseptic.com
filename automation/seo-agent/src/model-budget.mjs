...
import { assertModelCostReservation, assertModelRequestBudget, consumeBudget, resolveApprovedModelRoute } from './policy.mjs';
...
function reserve(usage, { prompt, maxOutputTokens, model }, budgets) {
  ...
  const tokenReservation = assertModelRequestBudget({ prompt, maxOutputTokens }, budgets);
  const costReservation = assertModelCostReservation({ model: approvedModel }, budgets);
  ...
  nextUsage = consumeBudget(nextUsage, 'maxModelTokens', tokenReservation.requested_tokens, budgets);
  nextUsage = consumeBudget(nextUsage, 'maxModelCostUsd', costReservation.reserved_max_cost_usd, budgets);
...
export function createModelRunLedger({ budgets = DEFAULT_BUDGETS } = {}) {
  ...
  return Object.freeze({
    reserve({ runId, prompt, maxOutputTokens, model }) {
      ...
      const result = reserve(runUsage.get(safeRunId) ?? {}, { prompt, maxOutputTokens, model }, budgets);

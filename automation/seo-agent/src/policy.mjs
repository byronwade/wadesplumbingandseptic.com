...
export function assertTruthState(value) {
  if (!TRUTH_STATES.includes(value)) throw new Error(`Unknown truth state: ${value}`);
  return value;
}
...
export function classifyUntrustedText(text) {
  if (typeof text !== 'string') {
    return { accepted: false, reason: 'UNTRUSTED_OR_UNSAFE_INSTRUCTION', matches: ['non-string-untrusted-input'] };
  }
  const matches = UNSAFE_CONTENT_PATTERNS.filter((pattern) => pattern.test(text)).map(String);
 ...
 */
export function assessContentProposal({ text, approvedEvidenceIds = [], existingPageDecision = 'UNKNOWN' } = {}) {
  if (typeof text !== 'string' || !text.trim()) {
    return Object.freeze({ decision: 'NO_ACTION', reason: 'EMPTY_OR_MALFORMED_PROPOSAL', matches: [] });
  }
  const untrusted = classifyUntrustedText(text);
  if (!untrusted.accepted) {
    return Object.freeze({ decision: 'NO_ACTION', reason: untrusted.reason, matches: untrusted.matches });
  }
  const matches = UNSUPPORTED_CONTENT_CLAIM_PATTERNS.filter((pattern) => pattern.test(text)).map(String);
  if (matches.length || !Array.isArray(approvedEvidenceIds) || approvedEvidenceIds.length === 0) {
    return Object.freeze({
      decision: 'REQUEST_FIRST_PARTY_EVIDENCE',
      reason: matches.length ? 'UNSUPPORTED_OR_DECEPTIVE_CONTENT_CLAIM' : 'MISSING_APPROVED_EVIDENCE',
      matches,
    });
  }
  if (!['REFRESH_EXISTING', 'CONSOLIDATE_EXISTING', 'NO_CHANGE', 'CREATE_JUSTIFIED'].includes(existingPageDecision)) {
    return Object.freeze({ decision: 'NO_ACTION', reason: 'MISSING_EXISTING_PAGE_DECISION', matches: [] });
  }
  return Object.freeze({ decision: existingPageDecision === 'NO_CHANGE' ? 'NO_ACTION' : 'PROPOSE_FOR_HUMAN_REVIEW', reason: null, matches: [] });
}

export function evaluateAdversarialPolicyFixture(fixture) {
  if (fixture?.schema_version !== '1.0' || !Array.isArray(fixture.cases)) throw new Error('Invalid adversarial policy fixture');
  const ids = new Set();
  return fixture.cases.map((entry) => {
    if (!entry?.id || ids.has(entry.id) || typeof entry.text !== 'string' || typeof entry.accepted !== 'boolean') {
      throw new Error(`Invalid adversarial policy case: ${entry?.id ?? 'unknown'}`);
    }
    ids.add(entry.id);
    const result = classifyUntrustedText(entry.text);
    if (result.accepted !== entry.accepted) throw new Error(`Adversarial policy case did not match expectation: ${entry.id}`);
...
export function authorizeAction(action, { humanApproval = false, draft = false, branch = '' } = {}) {
  ...
  if (action === 'create_draft_pull_request') {
    if (!humanApproval) throw new Error('Policy denied draft PR: human approval is required');
    if (!draft) throw new Error('Policy denied PR: only draft pull requests are permitted');
    if (typeof branch !== 'string' || branch !== branch.trim() || branch === 'main' || branch.startsWith('refs/') || !branch.includes('/')) {
      throw new Error('Policy denied PR: a non-main feature branch is required');
    }
  }
  if (action !== 'read_only_research' && action !== 'create_draft_pull_request') {
    throw new Error(`Policy denied unknown action: ${action}`);
  }
  return true;
}

export function consumeBudget(usage, key, increment = 1, budgets = DEFAULT_BUDGETS) {
  const limit = budgets[key];
  if (!Number.isFinite(limit)) throw new Error(`Unknown budget: ${key}`);
  const next = (usage[key] ?? 0) + increment;
  if (next > limit) throw new Error(`Budget exhausted: ${key} (${next}/${limit})`);
  return { ...usage, [key]: next };
}

export function assertModelRequestBudget({ prompt, maxOutputTokens }, budgets = DEFAULT_BUDGETS) {
  if (typeof prompt !== 'string' || !prompt.trim()) throw new Error('Model prompt must be a non-empty string');
  if (!Number.isInteger(maxOutputTokens) || maxOutputTokens < 1) throw new Error('Model maxOutputTokens must be a positive integer');
  const estimatedInputTokens = Math.ceil(new TextEncoder().encode(prompt).length / 4);
  const requestedTokens = estimatedInputTokens + maxOutputTokens;
  if (requestedTokens > budgets.maxModelTokens) {
    throw new Error(`Model token budget exhausted before request (${requestedTokens}/${budgets.maxModelTokens})`);
  }
  return Object.freeze({ estimated_input_tokens: estimatedInputTokens, requested_max_output_tokens: maxOutputTokens, requested_tokens: requestedTokens, max_model_tokens: budgets.maxModelTokens });
}

export function resolveApprovedModelRoute(value = DEFAULT_MODEL_ROUTE) {
  if (typeof value !== 'string' || value !== value.trim() || !APPROVED_MODEL_ROUTES.includes(value)) {
...
export function resolveModelProfile(role, env = process.env) {
  ...
  const envKey = role === 'writing' ? 'SEO_AGENT_WRITER_MODEL' : role === 'independent_qa' ? 'SEO_AGENT_JUDGE_MODEL' : 'SEO_AGENT_MODEL';
  const primary = resolveApprovedModelRoute(env[envKey] ?? policy.primary);
  const fallbackValue = env[`${envKey}_FALLBACKS`];
  const fallbacks = (fallbackValue ? fallbackValue.split(',') : policy.fallbacks)
    .map((value) => value.trim())
    .filter(Boolean)
    .map((value) => resolveApprovedModelRoute(value));
  if (new Set(fallbacks).size !== fallbacks.length || fallbacks.includes(primary)) {
    throw new Error(`Model fallback policy for ${role} must contain unique approved routes distinct from the primary`);
  }
...
export function assertModelCostReservation({ model = DEFAULT_MODEL_ROUTE, reservationUsd = MODEL_COST_RESERVATION_USD } = {}, budgets = DEFAULT_BUDGETS) {
  const approvedModel = resolveApprovedModelRoute(model);
  if (!Number.isFinite(reservationUsd) || reservationUsd <= 0) throw new Error('Model cost reservation must be a positive number');
  if (reservationUsd > budgets.maxModelCostUsd) {
    throw new Error(`Model cost budget exhausted before request (${reservationUsd}/${budgets.maxModelCostUsd})`);
  }
  return Object.freeze({ model: approvedModel, reserved_max_cost_usd: reservationUsd, max_model_cost_usd: budgets.maxModelCostUsd });
}

...
test("Git-backed run history accepts only canonical validated manifests", async () => {
	await withTempRoot((root) => {
		const manifest = createAuditOnlyRun({
		...
		assert.deepEqual(collectRecordedRunIds({ repoRoot: root }), [
			"scheduled-audit-2026-07-30",
		]);
		writeManifest(root, "wrong-directory-001", {
			...manifest,
			run_id: "scheduled-audit-2026-07-31",
		});
		assert.throws(
			() => collectRecordedRunIds({ repoRoot: root }),
			/does not match/,
		);
	});
});

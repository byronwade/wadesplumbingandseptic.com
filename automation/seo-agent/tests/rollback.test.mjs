...
import {
	assertHumanRollbackDrill,
	createHumanRollbackDrill,
...
test("rollback drill is human-only, provenance-bound, and rejects autonomous execution", () => {
	...
	assert.throws(
		() =>
			assertHumanRollbackDrill({
				...record,
				payload: { ...record.payload, automated_action_attempted: true },
			}),
		/prohibit autonomous rollback/,
	);
	assert.throws(
		() => createHumanRollbackDrill({ rollbackTarget: "agent_rollback" }),

...
export default defineTool({
	description:
		"Generate a bounded analysis only when AI Gateway credentials are available.",
	...
	async execute({ runId, prompt, maxOutputTokens = 1200 }) {
		...
		if (!process.env.AI_GATEWAY_API_KEY && !process.env.VERCEL_OIDC_TOKEN) {
			throw new Error(
				"AI Gateway is not configured; record BLOCKED_MISSING_CREDENTIALS instead of substituting a model response.",
			);
		}
		const result = await generateText({
			...
			prompt,
			maxOutputTokens,
			...resolveGatewayModelOptions("orchestration"),
		});

...
- Keep sidecar dependencies and `package-lock.json` local to this directory. The repository-root `vercel.json` declares this service as `eve_seo_agent` with root `automation/seo-agent` and reserved prefix `/_internal/eve`; never put agent UI or `NEXT_PUBLIC_` secrets under that prefix. Services share a Vercel project, so use Vercel OIDC/Connect where possible and treat any static third-party credential as project-wide exposure requiring owner approval.

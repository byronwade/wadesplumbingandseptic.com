...
const RelatedArticlesSection = dynamic(() => import("@/components/sections/RelatedArticlesSection"), { ssr: false });
const Sidebar = dynamic(() => import("@/components/sections/Sidebar"), { ssr: false });
const SocialBar = dynamic(() => import("@/components/sections/SocialBar"), { ssr: false });

import { getTipDetails } from "@/actions/getTips";

const cachedGetTipDetails = cache(getTipDetails);

function generateMetadataForTip(tipDetails, parentImages) {
  if (!tipDetails) {
    return {
...
export async function generateMetadata({ params }, parent) {
  ...
  const tipDetails = await cachedGetTipDetails(slug);
  const previousImages = (await parent).openGraph?.images || [];
  return generateMetadataForTip(tipDetails, previousImages);
}

export async function generateStaticParams() {
  // Implement this function to generate static params for all your blog posts
  // This will enable ISR for your blog pages
  // Example:
  // const allSlugs = await getAllTipSlugs();
  // return allSlugs.map((slug) => ({ slug: slug.split('/') }));
}

export default async function BlogPage({ params }) {
  const slug = params.slug.join("/");
  const postDetails = await cachedGetTipDetails(slug);
        ...
        <div className="relative">
          <section className="w-full h-[300px] sm:h-[400px] md:h-[460px] xl:h-[537px] relative">
            <Image 
              ...
              fill 
              src={postDetails.featuredImage?.[0]?.sourceurl || "/placeholder.webp"} 
              alt={postDetails.featuredImage?.[0]?.alttext || "placeholder image"} 
            />
          </section>
